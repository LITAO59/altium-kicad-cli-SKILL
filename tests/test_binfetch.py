"""Fully-offline tests for :mod:`altium_kicad_cli.drivers._binfetch`.
...
def test_resolve_path_first(monkeypatch, tmp_path):
    ...
    assert str(got).replace(os.sep, "/") == "/usr/local/bin/nlbn"


def test_resolve_cache_hit(monkeypatch, tmp_path):
    monkeypatch.setattr(shutil, "which", lambda name: None)
    exe = tmp_path / "bin" / f"nlbn-{_binfetch.NLBN_TAG}" / _binfetch._exe_name("nlbn")
